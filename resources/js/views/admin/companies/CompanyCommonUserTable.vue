...
<div class="card card-flush">
    ...
    <div class="card-body pt-6">
        ...
        <div 
        v-if="links.length && currentUsers.length"
        class="row mt-7 pagination-bottom">
...
export default {
    name: "company-common-user-table",
    ...
    methods: {
        ...
        fetch(isRefresh = null, fetchUrl = null) {
            this.current_url = fetchUrl ? fetchUrl : `/companies/${this.company.id}/common-users`;
            const params =  {
                params: { search: this.search, per_page: 20 }
            };
        ...
        appoint() {
            Swal.fire({
                title: "Assign Company Coordinator(s)?",
                html: `Are you sure to assigned the selected user(s)?
                    <br /> <br />
                    <i>Note: <b>${this.selectedUsers.length}</b> user(s) will be assigned as coordinators in <b>${this.company.company}</b>.</i>
                `,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    axios.post(`/companies/${this.company.id}/appoint`, data)
                        .then((response) => {
                            this.fetch();
                            this.$emit("appointed");
                            Swal.fire("User(s) Assigned", "User(s) has been assigned as coordinators", "success");
                            this.selectedUsers = [];
                        }).catch((error) => {
                            this.parseError(error, null, {});                            
                        }).finally(() => {
